...
IMPLS = awk bash c d clojure coffee cpp crystal cs erlang elixir es6 \
	...
	python r racket rpython ruby rust scala swift tcl vb vimscript scalapony
...
scalapony_STEP_TO_PROG =   scalapony/$($(1)).scala
...
scalapony_RUNSTEP = sbt 'run-main $($(1))$(if $(3), $(3),)'
